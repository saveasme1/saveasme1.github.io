...
# assetUrl rewrite for relative paths
old_asset = '''  function assetUrl(value) {
    const path = String(value || "").trim();
    if (!path) return "";
    if (/^https?:\\/\\//i.test(path)) return path;
    // Same-origin on hand-made.jp / github.io (mirrors private gongbang uploads).
    return `/${path.replace(/^\\/+/, "")}`;
  }'''
new_asset = '''  function assetUrl(value) {
    const path = String(value || "").trim();
    if (!path) return "";
    if (/^https?:\\/\\//i.test(path)) return path;
...
if old_asset in g:
    g = g.replace(old_asset, new_asset)
...
if old_fetch not in g:
    raise SystemExit("fetchPublicItems missing")
...
print("groupbuy ok", "LIVE_API" in g, "boards/groupbuy/publish" in g)
