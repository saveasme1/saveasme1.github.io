...
const CACHE_VERSION = "hx-pwa-v20260810-gbcal41";
...
function offlineImageResponse() {
  const svg =
    ...
    '<text x="320" y="300" text-anchor="middle" fill="#c4bdb4" font-family="sans-serif" font-size="28">?¤í”„?¼ì¸</text>' +
    '<text x="320" y="348" text-anchor="middle" fill="#8a837b" font-family="sans-serif" font-size="20">?„ì§ ìºì‹œ?˜ì? ?Šì? ?´ë?ì§€</text>' +
    "</svg>";
...
async function offlineShell() {
  return (
    ...
    new Response(
      "<!doctype html><meta charset=utf-8><title>?¤í”„?¼ì¸</title><body style='font-family:sans-serif;padding:24px;background:#161513;color:#fff'><h1>?¤í”„?¼ì¸</h1><p>?€?´íŒŒ?´ì—???±ì„ ?¤ì‹œ ?´ì–´ ?°ì´?°ë? ì¤€ë¹„í•´ ì£¼ì„¸??</p></body>",
...
self.addEventListener("fetch", (event) => {
  ...
  // HTML / JS / CSS must be network-first ??stale SW cache was killing ê°€ê²©ì¶”??
    ...
    matchIgnoringSearch(req).then((cached) => {
      const network = fetch(req)
        .then((res) => {
          ...
          // Avoid serving HTML/404 as an <img> ????“ì»¯
