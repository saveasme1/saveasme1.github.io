/* ë³??¤ë¦¬?°ì? PWA ??offline shell + image runtime cache + status events */
const CACHE_VERSION = "hx-pwa-v20260728-pwa46";
...
function offlineImageResponse() {
  const svg =
    ...
    '<text x="320" y="348" text-anchor="middle" fill="#8a837b" font-family="sans-serif" font-size="20">?„ì§ ?€?¥ë˜ì§€ ?Šì? ?´ë?ì§€</text>' +
    "</svg>";
...
async function offlineShell() {
  return (
    ...
    new Response(
      "<!doctype html><meta charset=utf-8><title>?¤í”„?¼ì¸</title><body style='font-family:sans-serif;padding:24px;background:#161513;color:#fff'><h1>?¤í”„?¼ì¸</h1><p>?€?´íŒŒ?´ì—???±ì„ ??ë²??´ì–´ ?°ì´?°ë? ì¤€ë¹„í•´ ì£¼ì„¸??</p></body>",
    ...
    matchIgnoringSearch(req).then((cached) => {
      const network = fetch(req)
        .then((res) => {
          ...
          // Avoid serving HTML/404 as an <img> ???¡ë°•
